        ...
        initedPromise.then(
          function(client) {
            ...
            client.GetStopDataByRoute(args, function(err, result) {
              if (err || !result.GetStopDataByRouteResult.diffgram) {
                reject(err);
              } else {
				  resolve({
					  route: route,
        ...
        initedPromise.then(
          function(client) {
            ...
            client.GetRealTimeStopData(args, function(err, result) {
              if (err || !result.GetRealTimeStopDataResult.diffgram) {
                reject(err);
              } else {
				const response = {
					stopId: stopId,
					departures: result.GetRealTimeStopDataResult.diffgram.DocumentElement.StopData
				};
                resolve(response);
              }
            });
